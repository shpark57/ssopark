...
export default function CartList(){
    ...
    const ontSelect = () => {
        ...
        if(loggedIn){
            axios.get( process.env.REACT_APP_SERVER_HOST + '/cart' , {params : {user_id: user.id , _rel : 'product' }})
                .then(res =>  {
                    ...
                    if(localCartList){
                        ...
                        cartList.forEach( (row:CartProps,index:number) =>{
                            ...
                            if(findIndex != -1){
                                ...
                                axios.post( process.env.REACT_APP_SERVER_HOST + "/Cart", res.data[findIndex])
                                    .catch((error) =>  {console.log("장바구니 수정 오류")});
                            }else{
                                res.data = [...res.data , cartList[index]]
                                axios.post( process.env.REACT_APP_SERVER_HOST + "/Cart", cartList[index])
                                    .catch((error) =>  {console.log("장바구니 수정 오류")});
                            }
                        })
    ...
    const handleCntChange = (event: ChangeEvent<HTMLInputElement>)=> {
        ...
        if(loggedIn){
            axios.post( process.env.REACT_APP_SERVER_HOST + "/Cart", updatedItems[  Number(event.target.name) ])
                .catch((error) =>  {console.log("장바구니 수정 오류")});
        }else{
            let cartLocalStorage = window.localStorage;
            let localCartList =  cartLocalStorage.getItem("localCartList")
    ...
    const handleCntClick = (row:any,add:number) => () => {
        ...
        setTableData(updatedItems);
        if(loggedIn){
            axios.post( process.env.REACT_APP_SERVER_HOST + "/Cart", updatedItems[  Number( row.id) ])
                .catch((error) =>  {console.log("장바구니 수정 오류")});
        }else{
            let cartLocalStorage = window.localStorage;
            let localCartList =  cartLocalStorage.getItem("localCartList")
