...
const MoviesView = () =>{  
    ...
    const getMovie = () => {
        axios.get( process.env.REACT_APP_SERVER_HOST + '/api/Movies/'+id+'?use_yn=Y')
        .then(res =>{
            setMovie(res.data)
        })
    }
    ...
    const likeClick = async  (event: React.FormEvent<HTMLButtonElement>) => {
        ...
        if(likeCheck.data.length ==0) {
            ...
            if(likeCheck.data[0].like_type == target_type){
                const likeDel =  await axios.delete( process.env.REACT_APP_SERVER_HOST + '/api/Likes/'+likeCheck.data[0].id)
