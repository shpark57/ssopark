...
const MoviesView = () =>{  
    ...
    const getMovie = () => {
        axios.get( '/api/Movies/'+id+'?use_yn=Y')
        .then(res =>{
            setMovie(res.data)
        })
    }
    ...
    const getVideoUrl = () =>{
        axios.get( '/api/Files?parent_id='+ id +'&Type=Movies&type_detail=video&_limit=1')
        .then(res=>{
    ...
    const likeClick = async  (event: React.FormEvent<HTMLButtonElement>) => {
        ...
        const likeCheck =  await axios.get( '/api/Likes?parent_id='+id+'&type=Movies&rgstr_id='+user.user_id)       // 중복 등록인지 체크

        if(likeCheck.data.length ==0) {
            const saveLike =  await axios.post( '/api//Likes',{parent_id:id ,type:'Movies' ,rgstr_id : user.user_id, like_type:target_type }) //좋아요 , 싫어요 등록
        ...
        const getLikeCnt =  await axios.get( '/api/Likes?parent_id='+id+'&type=Movies&like_type='+target_type)
        let params:any ={}
        params[target_type] = getLikeCnt.data.length
        axios.patch( '/api//Movies/'+id,params).then(res=>{
            getMovie()
