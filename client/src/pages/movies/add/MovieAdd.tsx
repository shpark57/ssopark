...
const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
    setLoading(true);  
    const resMovies = await axios.post( process.env.REACT_APP_SERVER_HOST_API + "/Movies" , params)
    ...
    axios.post( process.env.REACT_APP_SERVER_HOST_API + '/fileService/upload/Movies',formData ,config) // Movies 타입은 테이블명. 이게 폴더명으로 변경 됨
    .then(res=>{
