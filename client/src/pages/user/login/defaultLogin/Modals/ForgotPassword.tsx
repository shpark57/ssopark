...
const ForgotPassword = () => {
  ...
  const handleSubmit = async(event: React.FormEvent<HTMLFormElement>) => {
    ...
    if(radioValue == 'id'){
      axios.get( process.env.REACT_APP_CLIENT_HOST + "/api/Users?email="+data.get('email'))
          .then(res=>{
              ...
              showModal({
                ...
                modalProps: {
                  ...
                  handleConfirm: () => {
                    ...
                    axios.post( process.env.REACT_APP_CLIENT_HOST + "/api/sendmail",mailParam)
                        .then(res=>{
                          showModal({
      ...
      axios.get( process.env.REACT_APP_CLIENT_HOST + "/api/Users?email="+data.get('email')+"&user_id="+data.get('id'))
          .then(res=>{
            if(res.data.length == 0){
              ...
              showModal({
                ...
                modalProps: {
                  ...
                  handleConfirm: () => {
                    ...
                    axios.patch( process.env.REACT_APP_CLIENT_HOST + "/api/password/change/"+ res.data[0].id ,  {password :new_password , mdfr_time : Time.getTimeString() , mdfr_id : 'system' } )
                        .then((response) => {
                          var mailParam = {
                            name  : "비밀번호 초기화" ,
                            email : res.data[0].email ,
