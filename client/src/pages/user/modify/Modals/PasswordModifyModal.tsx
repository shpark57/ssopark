...
const PasswordModifyModal:React.FC<PasswordModifyModalProps> = ({user_id}) => {
  ...
  const handleSubmit = async(event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await axios.post( process.env.REACT_APP_SERVER_HOST + "/api/password/check",{user_id:user_id,password:password})
    if(res.data.check){

      axios.patch( process.env.REACT_APP_SERVER_HOST + "/api/password/change/"+ res.data.id ,  {password :new_password , mdfr_time : Time.getTimeString() , mdfr_id : user.user_id } )
      .then((response) => { 
        showModal({
