...
const ProductsView = () =>{
    ...
    const getProduct = () => {
        axios.get( process.env.REACT_APP_CLIENT_HOST + '/api/Products/'+id+'?use_yn=Y')
        .then(res =>{
            setProduct(res.data)
        })
    }
    ...
    const likeClick = async  (event: React.FormEvent<HTMLButtonElement>) => {
        ...
        const likeCheck =  await axios.get( process.env.REACT_APP_CLIENT_HOST + '/api/Likes?parent_id='+id+'&type=Products&rgstr_id='+user.user_id)       // 중복 등록인지 체크

        if(likeCheck.data.length ==0) {
            const saveLike =  await axios.post( process.env.REACT_APP_CLIENT_HOST + '/api/Likes',{parent_id:id ,type:'Products' ,rgstr_id : user.user_id, like_type:target_type }) //좋아요 , 싫어요 등록
        ...
        const getLikeCnt =  await axios.get( process.env.REACT_APP_CLIENT_HOST + '/api/Likes?parent_id='+id+'&type=Products&like_type='+target_type)
        let params:any ={}
        params[target_type] = getLikeCnt.data.length
        axios.patch( process.env.REACT_APP_CLIENT_HOST + '/api/Products/'+id,params).then(res=>{
            getProduct()
    ...
    const addCart = async () =>{
        if(loggedIn){
            axios.get( process.env.REACT_APP_CLIENT_HOST + "/api/Cart?user_id="+user.id+"&product_id="+product?.id)
                .then(res =>{
                    let params ={
                        user_id : user.id,
                    ...
                    axios.post( process.env.REACT_APP_CLIENT_HOST + "/api/Cart", params)
                        .then(res=>{
                            showModal({
                                modalType: "AlertModal",
                                modalProps: {
