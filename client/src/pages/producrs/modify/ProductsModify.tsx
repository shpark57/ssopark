...
export default function ProductsModify() {
    ...
    try {
      ...
      if(imgFiles.length > 0){
        ...
        formData.append('size', String(imgFiles[0].object.size))
         ...
         let res = await axios.post( process.env.REACT_APP_CLIENT_HOST + '/api/fileService/tuiHook/Products',formData ,config)
         params.title_img =res.data.message


      }

      await axios.put( process.env.REACT_APP_CLIENT_HOST + "/api/Products/"+ id , params)
  ...
  const onUploadImage = async (blob:any, callback:any) => {
    ...
    axios.post( process.env.REACT_APP_CLIENT_HOST + '/api/fileService/tuiHook/Products',formData ,config)
        .then(res => {
