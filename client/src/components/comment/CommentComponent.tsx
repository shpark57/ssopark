...
const CommentComponent:React.FC<CommentProps> = ({parent_id,type}) => {
    ...
    useEffect(() => {
        axios.get( process.env.REACT_APP_SERVER_HOST + '/Comment?parent_id='+parent_id)
        .then(res =>{
    ...
    useEffect(() => {
         axios.get( process.env.REACT_APP_SERVER_HOST + '/Comment?parent_id='+parent_id+'&type='+type+'&_rel=children&parent_comment_id=null&_sort=rgstr_time&_order=desc')
        .then(res =>{
